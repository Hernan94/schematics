...
export class <%= classify(name) %>Component   <T> {
  ...
  constructor(private commonService: CommonService,private messageService: MessageService,private confirmationService: ConfirmationService,) {}
...
async  ngOnInit() {
  ...
  async get<%= classify(name) %>s(){
    await this.commonService.get('product')
         .then((response : T ) => {
  ...
  async onSubmit() {
  await this.commonService.post('<%= name %>',this.form.value)
      .then((response : T )=> {
  ...
  async editRecord() {
    await this.commonService.update(`'<%= name %>'/${this.id}`,this.form.value,)
    .then((response : T ) => {
        let product = response as any;
        const index = this.data.findIndex(product => product.id === this.id);
        if (index !== -1) {
          this.data[index] = product;
        }
      this.popUp("Update record",'success',`El producto "${product?.title}" se actualizó correctamente.`);
    })
    .catch((error: any) => {
  ...
  async deleteRecord(id: unknown) {
    this.delete =true;
    await this.commonService.delete(`'<%= name %>'/${id}`)
    .then((response : T )=> {
